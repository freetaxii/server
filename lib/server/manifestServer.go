...
func (ezt *STIXServerHandlerType) ManifestServerHandler(w http.ResponseWriter, r *http.Request) {
	...
	log.Infoln("INFO: Found Request on the Manifest Server Handler from", r.RemoteAddr, "for collection:", ezt.CollectionID)
	...
	results, err := ezt.DS.GetManifestData(q)
	...
	if err != nil {
		...
		ezt.Resource = results.ManifestData
